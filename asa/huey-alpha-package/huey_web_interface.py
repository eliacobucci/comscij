...
def get_available_concepts(huey, min_mass: float = 0.1, max_concepts: int = 200) -> List[str]:
    ...
    try:
        ...
        if hasattr(huey.network, 'speakers') and huey.network.speakers:
            for speaker in huey.network.speakers:
                if hasattr(huey.network, 'analyze_speaker_self_concept'):
                    ...
                    if mass >= min_mass:
                        concept_masses.append((f"Speaker_{speaker}", mass))
        ...
        return concepts if concepts else ["No concepts available"]
                ...
                if connections:
                    ...
                    if selected_concept != 'None':
                        ...
                        if selected_id:
                            ...
                            # Add connection lines with proportional thickness if enabled
                            if show_connection_lines:
                                for conn in connections:
                                    # Only show connections involving the selected concept
                                    if selected_id in [conn['from_id'], conn['to_id']]:
                                        # Direct proportional scaling
                                        line_width = conn['strength'] * 5
                                        
                                        # Grey color with alpha based on strength - make lines visible
                                        alpha = max(0.7, min(1.0, 0.7 + (conn['strength'] * 0.3)))  # Scale alpha from 0.7 to 1.0
                                        grey_intensity = int(80 + (120 * conn['strength']))  # Scale grey from medium to light
                                        ...
                                        fig_highlighted.add_trace(go.Scatter3d(
                                            ...
                                            hovertemplate=f'Connection strength: {conn["strength"]:.3f}<extra></extra>',
                                            showlegend=False,
                                            name="connection"
                                        ))
