...
from huey_fortran_interface import HueyFortranInterface
...
class ExperimentalNetwork:
    ...
    def __init__(self, window_size=5, max_neurons=100, use_fortran_acceleration=True):
        ...
        self.use_fortran_acceleration = use_fortran_acceleration
        self.fortran_interface = None
        if use_fortran_acceleration:
            try:
                self.fortran_interface = HueyFortranInterface(max_neurons=max_neurons)
                print(f"🚀 HUEY++ FORTRAN ACCELERATION ENABLED")
            except Exception as e:
                print(f"⚠️  Fortran acceleration failed to initialize: {e}")
                print(f"   Falling back to standard Python implementation")
                self.use_fortran_acceleration = False
    ...
    def _hebbian_learning(self, window_neurons):
        ...
        # Use Fortran-accelerated implementation if available
        if self.use_fortran_acceleration and self.fortran_interface:
            return self._hebbian_learning_fortran(window_neurons)
        
        # Fallback to original Python implementation
        return self._hebbian_learning_python(window_neurons)
    
    def _hebbian_learning_fortran(self, window_neurons):
        """Fortran-accelerated Hebbian learning implementation."""
        ...
        stats = self.fortran_interface.hebbian_update_batch(window_neurons, self)
        
        # Optional performance logging
        if hasattr(self, '_log_performance') and self._log_performance:
            print(f"   Fortran kernel: {stats['updates_applied']} updates in {stats['computation_time']:.4f}s")
    ...
    def _apply_activation_decay(self):
        ...
        # Use Fortran-accelerated implementation if available
        if self.use_fortran_acceleration and self.fortran_interface:
            stats = self.fortran_interface.apply_activation_decay_batch(self)
            if hasattr(self, '_log_performance') and self._log_performance:
                print(f"   Decay kernel: {stats['neurons_processed']} neurons in {stats['computation_time']:.4f}s")
        else:
            # Fallback to original Python implementation
    ...
    def _calculate_all_activations(self, window_neurons):
        ...
        # Use Fortran-accelerated implementation if available
        if self.use_fortran_acceleration and self.fortran_interface:
            stats = self.fortran_interface.calculate_activations_batch(window_neurons, self)
            if hasattr(self, '_log_performance') and self._log_performance:
