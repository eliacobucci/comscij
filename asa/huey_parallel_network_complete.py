...
from huey_threaded_interface import HueyThreadedInterface
...
class ExperimentalNetwork:
    ...
    def __init__(self, window_size=5, max_neurons=100, use_fortran_acceleration=True):
        """
        ...
        Args:
            ...
            use_fortran_acceleration (bool): Enable Fortran acceleration for computational kernels
        """
        ...
        # HueyTime temporal learning integration
        self.huey_time = None
        self.use_temporal_learning = False  # Flag to enable HueyTime vs sliding windows
        self.synaptic_strengths = {}  # Directed connections from HueyTime
        
        # Huey++ Fortran acceleration
        self.use_threaded_acceleration = use_fortran_acceleration  # Reuse parameter for threading
        self.threaded_interface = None
        if use_fortran_acceleration:
            try:
                self.threaded_interface = HueyThreadedInterface(max_neurons=max_neurons)
                print(f"🚀 HUEY|| THREADED ACCELERATION ENABLED")
            except Exception as e:
                print(f"⚠️  Threaded acceleration failed to initialize: {e}")
                print(f"   Falling back to standard Python implementation")
                self.use_threaded_acceleration = False
        ...
        self._log_performance = False
        self.performance_stats = {
            'hebbian_calls': 0,
            'activation_calls': 0,
            'decay_calls': 0,
            'total_fortran_time': 0.0
        }
        
    def enable_temporal_learning(self, method="lagged", max_lag=8, tau=3.0, eta_fwd=1e-2, eta_fb=2e-3):
        """Enable HueyTime temporal learning instead of sliding windows."""
        # Build vocabulary from current neurons
        vocab = {word: neuron_id for word, neuron_id in self.word_to_neuron.items()}
        ...
        self.huey_time = HueyTime(config)
        self.use_temporal_learning = True
        
        print(f"🕐 HUEY+ TEMPORAL LEARNING ENABLED:")
        print(f"   Method: {method}")
        print(f"   Max lag: {max_lag}")
        print(f"   Time decay (tau): {tau}")
        print(f"   Learning rates: fwd={eta_fwd}, fb={eta_fb}")
        print(f"   Vocabulary size: {len(vocab)}")
    
    def enable_performance_logging(self, enabled: bool = True):
        """Enable detailed performance logging for Fortran kernels."""
        self._log_performance = enabled
        if enabled:
            print("📊 Fortran kernel performance logging enabled")
        else:
            print("📊 Performance logging disabled")
    
    def get_performance_summary(self) -> Dict[str, float]:
        """Get comprehensive performance summary including Fortran kernel stats."""
        summary = self.performance_stats.copy()
        
        if self.fortran_interface:
            fortran_stats = self.fortran_interface.get_performance_stats()
            summary.update({
                'fortran_kernel_calls': fortran_stats['kernel_calls'],
                'fortran_total_time': fortran_stats['total_time'],
                'fortran_avg_time_per_call': fortran_stats['average_time_per_call'],
                'using_fortran_kernels': fortran_stats['using_fortran']
            })
        
        # Calculate derived metrics
        total_calls = summary.get('hebbian_calls', 0) + summary.get('activation_calls', 0)
        if total_calls > 0:
            summary['efficiency_ratio'] = summary.get('fortran_total_time', 0) / total_calls
        
        return summary
    
    def _sync_from_huey_time(self):
        """Sync Huey's connection matrices from HueyTime weights."""
        if not self.huey_time:
            return
            
        # Get directed weight matrix W and symmetric matrix S
        W = self.huey_time.export_W()
        S = self.huey_time.export_S(mode="avg")
        
        # Update inertial_mass from symmetric matrix (for compatibility with existing code)
        self.inertial_mass = {}
        V = W.shape[0]
        for i in range(V):
            for j in range(i+1, V):  # Only upper triangle for symmetric
                if S[i, j] > 1e-6:  # Only significant connections
                    self.inertial_mass[(i, j)] = S[i, j]
        
        # Update synaptic_strengths from directed matrix (for causality analysis)
        self.synaptic_strengths = {}
        for i in range(V):
            for j in range(V):
                if W[i, j] > 1e-6:  # Only significant connections
                    self.synaptic_strengths[(i, j)] = W[i, j]
        
    def process_text_stream(self, text):
        """
        Process text as a stream through a moving window or HueyTime temporal learning.
        ...
        if self.use_temporal_learning and self.huey_time:
            # Use HueyTime temporal learning
            self.huey_time.update_doc(words)
            # Update our connection matrices from HueyTime
            self._sync_from_huey_time()
        else:
    ...
    def _hebbian_learning(self, window_neurons):
        ...
        Uses Fortran acceleration when available for significant performance gains.
        
        Args:
            window_neurons (list): List of neuron indices in current window
        """
        
        # Use threaded-accelerated implementation if available
        if self.use_threaded_acceleration and self.threaded_interface:
            return self._hebbian_learning_threaded(window_neurons)
        
        # Fallback to original Python implementation
        return self._hebbian_learning_python(window_neurons)
    
    def _hebbian_learning_threaded(self, window_neurons):
        """Threaded-accelerated Hebbian learning implementation."""
        
        # Apply decay first using standard method (could be Fortran-accelerated later)
        self._decay_unused_masses()
        self._decay_all_connections(window_neurons)
        
        # Use high-performance Fortran kernel for the learning updates
        stats = self.threaded_interface.hebbian_update_batch(window_neurons, self)
        
        # Optional performance logging
        if hasattr(self, '_log_performance') and self._log_performance:
            threading_info = f" (threading: {'yes' if stats.get('threading_used') else 'no'})"
            print(f"   Threading kernel: {stats['updates_applied']} updates in {stats['computation_time']:.4f}s{threading_info}")
        
        # Periodical pruning (same as before)
        if self.processed_words % 100 == 0:
            pruned_count = self._prune_weak_connections(threshold=0.01)
            if pruned_count > 0 and (pruned_count >= 5 or self.processed_words % 100 == 0):
    ...
    def _apply_activation_decay(self):
        ...
        Uses Fortran acceleration when available for vectorized operations.
        """
        
        # Use threaded-accelerated implementation if available
        if self.use_threaded_acceleration and self.threaded_interface:
            stats = self.threaded_interface.apply_activation_decay_batch(self)
            if hasattr(self, '_log_performance') and self._log_performance:
                threading_info = f" (threading: {'yes' if stats.get('threading_used') else 'no'})"
                print(f"   Decay kernel: {stats['neurons_processed']} neurons in {stats['computation_time']:.4f}s{threading_info}")
        else:
            # Fallback to original Python implementation
            self._apply_activation_decay_python()
    
    def _apply_activation_decay_python(self):
        """Original Python activation decay (fallback)."""
    ...
    def _calculate_all_activations(self, window_neurons):
        ...
        # Use threaded-accelerated implementation if available
        if self.use_threaded_acceleration and self.threaded_interface:
            stats = self.threaded_interface.calculate_activations_batch(window_neurons, self)
            if hasattr(self, '_log_performance') and self._log_performance:
                print(f"   Activation kernel: {stats['neurons_updated']} neurons in {stats['computation_time']:.4f}s")
        else:
            # Fallback to original Python implementation
            self._calculate_all_activations_python(window_neurons)
    
    def _calculate_all_activations_python(self, window_neurons):
        """Original Python activation calculation (fallback)."""
        import math
    ...
    def _pseudo_riemannian_embedding(self, connection_matrix, n_components=2):
        ...
        try:
            ...
            # Sort by eigenvalue in descending algebraic order (most positive first)  
