...
try:
    ...
    try:
        ...
        from huey_speaker_detector import HueySpeakerDetector
        USE_GPU = False
        st.warning("⚠️ GPU components not available, using CPU-only mode")
    except ImportError as e2:
        st.error(f"❌ Could not import Huey components: {e2}")
        st.stop()
...
def get_network(huey):
    """Get network object from either GPU or CPU version"""
    if USE_GPU:
        return huey  # GPU version is the network itself
    else:
        return huey.network  # CPU version has a .network attribute

def query_concepts_compat(huey, query):
    """Query concepts with compatibility for both GPU and CPU versions"""
    if USE_GPU:
        # GPU version: return basic network statistics
        network = get_network(huey)
        if query == "network_statistics":
            return {
                'total_neurons': getattr(network, 'neuron_count', 0),
                'total_connections': len(getattr(network, 'connections', {})),
                'speakers': list(getattr(network, 'speakers', {}).keys()) if hasattr(network, 'speakers') else [],
                'processing_rate': getattr(network, 'processing_rate', 0)
            }
        else:
            return {}
    else:
        # CPU version has the query_concepts method
        return huey.query_concepts(query)

def generate_analysis_compat(huey):
    """Generate analysis with compatibility for both GPU and CPU versions"""
    if USE_GPU:
        # GPU version: create a simple analysis structure
        network = get_network(huey)
        return {
            'network_summary': {
                'total_neurons': getattr(network, 'neuron_count', 0),
        ...
        # CPU version has the full analysis method
        return huey._generate_comprehensive_analysis()

def register_speakers_compat(huey, speakers_info):
    """Register speakers with compatibility for both GPU and CPU versions"""
    if USE_GPU:
        # GPU version uses add_speaker method
        if hasattr(huey, 'add_speaker'):
            # Handle various data formats
            if isinstance(speakers_info, list):
                for speaker_data in speakers_info:
                    if isinstance(speaker_data, tuple):
                        # If it's a tuple, use the first element as speaker ID
                        speaker_id = str(speaker_data[0]) if len(speaker_data) > 0 else 'Unknown'
                    elif isinstance(speaker_data, dict):
                        speaker_id = speaker_data.get('id', speaker_data.get('name', 'Unknown'))
                    else:
                        speaker_id = str(speaker_data)
                    
                    # Default self/other words for conversational analysis
                    self_words = ['i', 'me', 'my', 'myself']
                    other_words = ['you', 'your', 'yourself']
                    huey.add_speaker(speaker_id, self_words, other_words)
            else:
                for speaker_id, speaker_data in speakers_info.items():
                    # Default self/other words for conversational analysis
                    self_words = ['i', 'me', 'my', 'myself']
                    other_words = ['you', 'your', 'yourself']
                    huey.add_speaker(speaker_id, self_words, other_words)
    else:
        # CPU version has register_speakers method
...
def initialize_huey(max_neurons, window_size, learning_rate):
    ...
    if USE_GPU:
        # Use GPU-accelerated version for 20-50x speedup
        st.info("🚀 Initializing with GPU acceleration...")
        huey = HueyGPUConversationalNetwork(
            max_neurons=max_neurons,
            window_size=window_size,
            learning_rate=learning_rate,
            use_gpu_acceleration=True
        )
        # Add default speakers for GPU version
        huey.add_speaker("Human", ['i', 'me', 'my', 'myself'], ['you', 'your', 'yourself'])
        huey.add_speaker("AI", ['i', 'me', 'my', 'myself'], ['you', 'your', 'yourself'])
    else:
        # Fallback to CPU version
        st.warning("💻 Using CPU-only mode (slower)")
...
def process_uploaded_file(uploaded_file, huey, timeout_hours=2.0, exchange_limit=10000):
    ...
    try:
        ...
        register_speakers_compat(huey, result['speakers_info'])
        ...
        try:
            ...
            for i, (speaker_id, text) in enumerate(result['conversation_data']):
                ...
                if elapsed_time > MAX_PROCESSING_TIME:
                    ...
                    analysis_results = generate_analysis_compat(huey)
                    return {
                        'success': True,
                ...
                try:
                    network = get_network(huey)
                    if hasattr(network, 'process_speaker_text'):
                        network.process_speaker_text(speaker_id, text)
                    else:
                        # Fallback for different API
                        huey.process_speaker_text(speaker_id, text)
                except Exception as e:
...
def create_3d_concept_plot(huey, num_concepts, min_mass, connection_direction="both", eigenvector_type="right"):
    ...
    try:
        ...
        network = get_network(huey)
        st.write(f"- Network has **{len(getattr(network, 'neuron_to_word', {}))}** neurons")
        st.write(f"- Network has **{len(getattr(network, 'inertial_mass', {}))}** mass connections")
        
        if hasattr(network, 'inertial_mass') and getattr(network, 'inertial_mass', {}):
            sample_masses = list(getattr(network, 'inertial_mass', {}).items())[:3]
            st.write(f"- Sample mass entries: {sample_masses}")
        else:
            st.write(f"- **No inertial_mass found!**")
        
        if getattr(network, 'neuron_to_word', {}):
            sample_neurons = list(getattr(network, 'neuron_to_word', {}).items())[:5]
...
def main():
    ...
    with st.sidebar:
        ...
        if USE_GPU:
            ...
            st.markdown('<div style="font-size: 0.8em; color: #006400;">PyTorch MPS - 20-50x faster processing</div>', unsafe_allow_html=True)
        else:
            st.markdown("💻 **GPU Acceleration**: DISABLED") 
            st.markdown('<div style="font-size: 0.8em; color: #ff6600;">CPU-only mode - slower performance</div>', unsafe_allow_html=True)
    ...
    with upload_tab:
        ...
        network = get_network(st.session_state.huey)
        if hasattr(network, 'max_neurons') and network.max_neurons != max_neurons:
    ...
    if st.session_state.conversation_processed and st.session_state.analysis_results:
        ...
        with col3:
            # Get network stats
            stats = query_concepts_compat(huey, "network_statistics")
        ...
        with col5:
            ...
            if 'connection_stats' in stats and 'total_strength' in stats['connection_stats']:
                ...
                network = get_network(huey)
                network_strength = sum(getattr(network, 'connections', {}).values())
            ...
            if st.button("🔍 Analyze Associations"):
                ...
                if isinstance(concept_name, list) and len(concept_name) > 0:
                    ...
                    for concept in concept_name:
                        result = query_concepts_compat(huey, "strongest_associations")
