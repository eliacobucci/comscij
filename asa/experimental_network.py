class ExperimentalNetwork:
    ...
    def visualize_cognitive_space(self, figsize=(12, 10), show_connections=True, 
        ...
        # Print statistics
        print(f"\n🧠 COGNITIVE SPACE STATISTICS:")
        print(f"📊 Concepts visualized: {sum(1 for i in range(self.neuron_count) if i in self.neuron_to_word)}")
        if show_connections:
            print(f"🔗 Connections shown: {connections_drawn}/{len(self.connections)}")
        print(f"⚡ Activation range: {min(activations):.3f} - {max(activations):.3f}")
        print(f"🏋️ Mass range: {min(sphere_sizes):.0f} - {max(sphere_sizes):.0f}")
        
        if eigenvalues is not None and metric_signature is not None:
            pos_dims = metric_signature[0] if len(metric_signature) >= 1 else 0
            neg_dims = metric_signature[1] if len(metric_signature) >= 2 else 0
            zero_dims = metric_signature[2] if len(metric_signature) >= 3 else 0
            print(f"🌌 Metric signature: (+{pos_dims}, -{neg_dims}, 0{zero_dims})")
            print(f"📏 Eigenvalue range: {min(eigenvalues):.3f} to {max(eigenvalues):.3f}")
            if neg_dims > 0:
                print(f"⚡ Pseudo-Riemannian space detected: {neg_dims} timelike dimensions")
            else:
                print(f"📐 Riemannian space: all dimensions spacelike")
...
# Demo the basic windowing concept
