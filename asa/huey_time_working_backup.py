    ...
    if st.button("🎯 Create 3D Visualization"):
        ...
        with st.spinner("Creating 3D plot..."):
            ...
            try:
                ...
                # DIRECT SCALAR PRODUCTS METHOD: Double center the similarity matrix directly
                try:
                    # HueyTime synaptic weights are already scalar products - just double center them!
                    centering_matrix = np.eye(n) - np.ones((n, n)) / n
                    gram_matrix = centering_matrix @ similarity @ centering_matrix
