...
if uploaded_file:
    ...
    if st.button("🚀 Process with HueyTime"):
        with st.spinner("Processing..."):
            ...
            # Configure HueyTime with user-controlled learning rate and CONTEXT method for conversations
            eta_fb = learning_rate * 0.2  # feedback rate = 20% of forward rate
            config = HueyTimeConfig(vocab=vocab, method="context", eta_fwd=learning_rate, eta_fb=eta_fb)
            ...
            # CHATGPT-5's CORRECT APPROACH: Detect speaker boundaries and use context method
            # Find speaker transitions in the conversation
            speaker_boundaries = []
            for i, word in enumerate(words):
                # Look for speaker patterns like "FEYNMAN:", "WIENER:", etc.
                if ':' in word and word.replace(':', '').isupper():
                    if i > 0:  # Don't include the very first speaker as a boundary
                        speaker_boundaries.append(i - 1)  # Boundary AFTER previous word

            if len(speaker_boundaries) > 0:
                st.write(f"🗣️ Conversation detected! Found {len(speaker_boundaries)} speaker transitions")
                st.write(f"Using CONTEXT method with boundary down-weighting as ChatGPT-5 intended")
                huey_time.update_doc(words, boundaries=speaker_boundaries)
            else:
                st.write("📝 Single-speaker text detected, processing without boundaries")
